...
import inspect
print("************************************************************+")
print("JMetalWorker module file:", inspect.getfile(JMetalWorker))
print("JMetalWorker class:", JMetalWorker.JMetalWorker)
...
class MainWindow(QMainWindow):
    ...
    def execute_algorithm(self):
        ...
        jar_path = ".\ParetoInvest\jar\portfolio-6.2.3-SNAPSHOT-jar-with-dependencies.jar"

        # Load the contents of the UI components into the variables to execute.
        population_size = self.textboxes_problem['population_size'].text()
        num_studied = self.textboxes_problem['num_studied'].text()
        total_num = self.textboxes_problem['total_num'].text()
        frequency = self.freq_combo.currentText()
        markets_combo = self.markets_combo.currentText()
        increase = self.textboxes_problem['increase'].text()
        #window = self.textboxes['window'].text()
        NumEvals = self.textboxes_problem['Num_Evaluations'].text()
        ...
        seed = self.textboxes_problem['seed'].text()
        self.log.printAndLogger("seed", seed)
        
        if alg_combo == "MOEADDE":            

            cr = self.components_algs['cr'].text()
            f = self.components_algs['f'].text()            
            mutationDistributionIndex = self.components_algs['mutationDistributionIndex'].text()
            ...
            crossoverProbability = self.components_algs['crossoverProbability'].text()
            self.log.printAndLogger("crossoverProbability", crossoverProbability)
            crossoverDistributionIndex = self.components_algs['crossoverDistributionIndex'].text()
        ...
        # Generate string execution for cmd
        cmd_str = subprocess.list2cmdline(cmd)
        self.log.printAndLogger("CMD command:")
        self.log.printAndLogger("    -- " + cmd_str)
    ...
    def run_jmetal_logic(self):
        ...
        if len(self.df_Assets) > 0:
            ...
            population_size = int(self.textboxes_problem['population_size'].text())
            num_studied = int(self.textboxes_problem['num_studied'].text())
            total_num = int(self.textboxes_problem['total_num'].text())
            frequency = self.freq_combo.currentText()
            markets_combo = self.markets_combo.currentText()
            increase = int(self.textboxes_problem['increase'].text())
    ...
    def download_data(self):
        ...
        if not self.df_Assets is None and len(self.df_Assets) > 0:
            ...
            textbox_values = {key: textbox.text() for key, textbox in self.textboxes_problem.items()}
    ...
    def selectAssets(self, considerSizes=False):
        ...
        self.total_num = int(self.textboxes_problem['total_num'].text())
        self.frequency = self.freq_combo.currentText()
        self.window = self.window_combo.currentText()
        self.duration = self.textboxes_problem['duration'].text()
